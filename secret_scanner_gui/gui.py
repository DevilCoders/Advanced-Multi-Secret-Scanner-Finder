...
class SecretScannerGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.max_file_size_var = tk.IntVar(
            value=max(1, int(self.scanner.max_file_size / (1024 * 1024)))
        )
    ...
    def _build_options_panel(self) -> None:
        ...
        ttk.Label(options_frame, text="Max file size (MB):").grid(row=0, column=2, padx=(12, 4), sticky="w")
    ...
    def _apply_scanner_options(self) -> None:
        max_size = max(1, int(self.max_file_size_var.get())) * 1024 * 1024
    ...
    def _clear_detail_panel(self) -> None:
        ...
        self.detail_text.insert(
            "1.0", "Context preview will appear here when you select a finding."
        )
        self.detail_text.insert(
            "end", " Use the filters above to focus on critical signals."
        )
        self.detail_text.configure(state="disabled")
