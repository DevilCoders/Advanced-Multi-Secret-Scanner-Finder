...
class SecretScanner:
    ...
    def _scan_file(
        ...
        for pattern in SECRET_PATTERNS:
            for match in pattern.pattern.finditer(text):
                ...
                context = self._extract_context(text, line_start, line_end)
                findings.append(
                    Finding(
                        pattern=pattern,
                        file_path=path,
        ...
        if self.enable_entropy_checks:
            findings.extend(self._run_entropy_checks(text, path))
            findings.extend(self._keyword_sweeps(text, path))
    ...
    def _run_entropy_checks(self, text: str, path: Path) -> list[Finding]:
        findings: list[Finding] = []
        for match in _ENTROPY_REGEX.finditer(text):
    ...
    def _keyword_sweeps(self, text: str, path: Path) -> list[Finding]:
        findings: list[Finding] = []
        for match in _CREDENTIAL_KEYWORD_REGEX.finditer(text):
            line_number = text.count("\n", 0, match.start()) + 1
            line_start = text.rfind("\n", 0, match.start()) + 1
            line_end = text.find("\n", match.end())
            if line_end == -1:
